...
onmessage = function (e) {
	...
	if (type === 'createGeometry') {
		...
		const oceanMorphPositions = oceanGeometry.morphAttributes.position[0].array.buffer;
		const oceanMorphNormals = oceanGeometry.morphAttributes.normal[0].array.buffer;
...
function createGeometry(
	...
	for (let i = 0; i < vertices.count; i += 3) {
		...
		for (let j = 0; j < 3; j++) {
			...
			if (!move) {
				...
				const secondSeaHeight = biome.getSeaHeight(v.addScalar(100)) * 0.3 + seaHeight * 0.7;
		...
		normalizedHeight /= 3;
		normalizedHeight =
			Math.min(-normalizedHeight / biome.min, 0) + Math.max(normalizedHeight / biome.max, 0);
		...
		const steepness = Math.acos(Math.abs(temp.dot(planetOptions.shape == 'plane' ? planeUp : mid)));
		...
		temp.crossVectors(oceanB.clone().sub(oceanA), oceanC.clone().sub(oceanA)).normalize();
		...
		temp.crossVectors(oceanE.clone().sub(oceanD), oceanF.clone().sub(oceanD)).normalize();
		oceanMorphNormals.push(temp.x, temp.y, temp.z);
		oceanMorphNormals.push(temp.x, temp.y, temp.z);
		oceanMorphNormals.push(temp.x, temp.y, temp.z);

		// place vegetation
		for (let j = 0; biome.options.vegetation && j < biome.options.vegetation.items.length; j++) {
			...
			if (Math.random() < faceSize * (vegetation.density ?? 1)) {
				...
				if (vegetation.minimumHeight !== undefined && normalizedHeight < vegetation.minimumHeight) {
					continue;
				}
				...
				if (vegetation.maximumHeight !== undefined && normalizedHeight > vegetation.maximumHeight) {
					continue;
				}

				// discard if point is below or above slope limits
				if (vegetation.minimumSlope !== undefined && steepness < vegetation.minimumSlope) {
					continue;
				}
				if (vegetation.maximumSlope !== undefined && steepness > vegetation.maximumSlope) {
					continue;
				}
	...
	for (let i = 0; i < vertices.count; i += 3) {
		...
		const output = biome.vegetationHeightAndColorForFace(a, b, c, color, faceSideLength);
	...
	oceanGeometry.morphAttributes.position[0] = new Float32BufferAttribute(oceanMorphPositions, 3);
	oceanGeometry.morphAttributes.normal[0] = new Float32BufferAttribute(oceanMorphNormals, 3);
